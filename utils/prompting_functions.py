...
def get_sys_prompt(model_id):
    if model_id == "meta-llama/Meta-Llama-3.1-8B":
        ...
        system_prompt = '''You are an expert in Mathematics. Given the following math problem, provide a complete and detailed solution.
        ...
        system_prompt = '''You are an expert in Mathematics. Given the following math problem, provide a complete final solution but optimum.
...
def get_one_shot_prompt(dataset_split):
    if dataset_split == 'algebra':
        ...
        solution = '130'
    one_shot_prompt = f'''Problem: {example_part}
        Solution: {solution}
'''
    return (one_shot_prompt)

def get_one_shot_hint_prompt(dataset_split):
    if dataset_split == 'algebra':
        example_part = "What is the sum of all values of $y$ for which the expression $\\frac{y+6}{y^2-5y+4}$ is undefined?"
...
def get_one_shot_adv_prompt(dataset_split):
    if dataset_split == 'algebra':
        example_part = "What is the sum of all values of $y$ for which the expression $\\frac{y+6}{y^2-5y+4}$ is undefined?"
        solution = '-912'
    elif dataset_split == 'counting_and_probability':
...
def get_few_shot_adv_prompt(dataset_dir):
    few_shot_prompt = ""
    for json_file in os.listdir(dataset_dir):
        if json_file.endswith('.json'):
            file_path = os.path.join(dataset_dir, json_file)
            ...
            few_shot_prompt = few_shot_prompt + f"""
Problem: {problem}
Solution: {solution + f'{random.randint(0, 1000)}'}"""
    return (few_shot_prompt)
...
def chain_of_thought(dataset_split):
    if dataset_split == 'algebra':
        example_part = "What is the sum of all values of $y$ for which the expression $\\frac{y+6}{y^2-5y+4}$ is undefined?"
        solution = '''Let's break down the solution step by step: 
        ...
        example_part = "What is the coefficient of $x^8$ in the expansion of $(x-1)^9$?"
        solution = '''Step 1: **Understanding the Problem**: We are asked to find the coefficient of a specific term in the expansion of \\( (x + (-1))^9 \\) using the Binomial Theorem. 
        ...
        example_part = "A cube with an edge length of 4 units has the same volume as a square-based pyramid with base edge lengths of 8 units and a height of $h$ units. What is the value of $h$?"
        solution = '''Step 1: **Determine the Volume of the Cube**: The volume of the cube is given by \\( 4^3 \\). We calculate this as: \\[ 4^3 = 64 \\] So, the volume of the cube is 64 cubic units. 
        ...
        example_part = "Let $a$ and $b$ be nonzero real numbers such that\n\\[(2 - 7i)(a + bi)\\]is pure imaginary.  Find $\\frac{a}{b}.$"
        solution = '''Step 1: **Expand the Expression**: We are given the product \\( (2 - 7i)(a + bi) \\). We need to expand this expression: \\[ (2 - 7i)(a + bi) \\] Use the distributive property to expand: \\[ = 2 \\cdot (a + bi) - 7i \\cdot (a + bi) \\] \\[ = 2a + 2bi - 7ai - 7bi^2 \\] 
        ...
        example_part = "When the binary number $100101110010_2$ is divided by 4, what is the remainder (give your answer in base 10)?"
        solution = """Step 1: **Understand the Binary Number Representation**: The binary number \\( b_k b_{k - 1} \\dots b_2 b_1 b_0 \\) represents a decimal number where each digit \\( b_i \\) is multiplied by \\( 2^i \\). The decimal value of the binary number is given by: \\[ 2^k b_k + 2^{k - 1} b_{k - 1} + \\dots + 4b_2 + 2b_1 + b_0 \\] 
        ...
        example_part = "Find $r$ if $3(r-7) = 4(2-2r) + 4$."
        solution = """Step 1: **Expand Both Products**: We start with the equation involving products. Expanding both products, we get: \\[ 3r - 3 \\cdot 7 = 4 \\cdot 2 - 4 \\cdot 2r + 4 \\] 
        ...
        **Conclusion**: Therefore, the value of \\( r \\) is \\( \\boxed{3} \\)."""
        ...
        example_part = "Let $S$ be a region in the plane with area 10.  When we apply the matrix\n\\[\\begin{pmatrix} 2 & 1 \\\\ 7 & -3 \\end{pmatrix}\\]to $S,$ we obtain the region $S'.$  Find the area of $S'."
        solution = """Step 1: **Calculate the Determinant**: To find how a matrix scales the area of a region, we first compute the determinant of the matrix. Given the matrix: \\[ \\begin{vmatrix} 2 & 1 \\\\ 7 & -3 \\end{vmatrix} \\] the determinant is calculated as: \\[ (2 \\cdot -3) - (1 \\cdot 7) \\] which simplifies to: \\[ -6 - 7 = -13 \\] 
    ...
    CoT_prompt = f'''Problem: {example_part}
    Solution: {solution}
'''
    return (CoT_prompt)

def chain_of_thought_adverserial(dataset_split):
    if dataset_split == 'algebra':
        example_part = "What is the sum of all values of $y$ for which the expression $\\frac{y+6}{y^2-5y+4}$ is undefined?"
        solution = '''Step 1: **Understanding the Problem**: We are asked to find the coefficient of a specific term in the expansion of \\( (x + (-1))^9 \\) using the Binomial Theorem. 
        Step 2: **Recall the Binomial Theorem**: The Binomial Theorem states that for any positive integer \\( n \\) and any real numbers \\( a \\) and \\( b \\), the expansion of \\( (a + b)^n \\) is given by: \\[ (a + b)^n = \\sum_{k=0}^{n} \\binom{n}{k} a^{n-k} b^k \\] where \\( \\binom{n}{k} \\) is the binomial coefficient. 
...
def get_dir_adv_hint(dataset_split):
    if dataset_split == 'algebra':
        example_part = "What is the sum of all values of $y$ for which the expression $\\frac{y+6}{y^2-5y+4}$ is undefined?"
        hint = 'To solve this problem, first calculate the volume of the cube. Then, set up an equation equating the volume of the cube to the volume of the pyramid. Finally, solve for the height (h) of the pyramid. \n'
        solution = '5'
